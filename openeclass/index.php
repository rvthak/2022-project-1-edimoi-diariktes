...
try{
	$t_pdo_conn = new PDO("mysql:host=$mysqlServer;dbname=$mysqlMainDb", $mysqlUser, $mysqlPassword);
}catch (Exception $e){
...
if (isset($_SESSION['shib_uname'])) { // authenticate via shibboleth
	...
	if(!empty($submit)) {
		...
		/*$result_stm = db_query_safe("SELECT user_id, nom, username, password, prenom, statut, email, perso, lang
			FROM user WHERE username= ? ",
			array($uname),$mysqlMainDb,FALSE
		);*/
		$t_pdo_conn->exec("set names utf8mb4");
		$pdo_stm = $t_pdo_conn->prepare("SELECT user_id, nom, username, password, prenom, statut, email, perso, lang FROM user WHERE username= ?");
		$pdo_stm->bindParam(1, $uname);
		$pdo_stm->execute();
		...
		if (!isset($_COOKIE) or count($_COOKIE) == 0) {
			...
			while ($myrow = $pdo_stm->fetch(PDO::FETCH_ASSOC)) {
				$exists = 1;
				if(!empty($auth)) {
		...
		if (!isset($uid)) {
			...
			$pdo_stm = $t_pdo_conn->prepare("INSERT INTO loginout (loginout.idLog, loginout.id_user, loginout.ip, loginout.when, loginout.action) VALUES ('', ? ,? , NOW(), 'LOGIN')");
			$pdo_stm->bindParam(1, $uid); $pdo_stm->bindParam(2, $_SERVER[REMOTE_ADDR]);
			$pdo_stm->execute();
		}
...
if (isset($language)) {
		...
		if( $language!='english' && $language!='greek' && $language!='spanish' ){ $language='greek'; }
...
elseif ((isset($logout) && isset($uid)) OR (1==1)) {
	$logout = htmlspecialchars($logout, ENT_QUOTES);
