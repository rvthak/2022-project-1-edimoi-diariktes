...
if (isset($_REQUEST['do']) && $_REQUEST['do'] == "go") {
	...
	$email = htmlspecialchars($userName, ENT_QUOTES);
	...
	/*$res = db_query("SELECT user_id, nom, prenom, username, password, statut FROM user
			...
			db_query_safe($sql,array($s['user_id'],md5($new_pass),$new_pass), $mysqlMainDb);*/
	$res_stm = db_query_safe("SELECT user_id, nom, prenom, username, password, statut FROM user
			WHERE email = ?
			AND BINARY username = ?",
			array($email,$userName), $mysqlMainDb, FALSE);

        $found_editable_password = false;
	if ($res_stm->rowCount() == 1) {
		$text = $langPassResetIntro. $emailhelpdesk;
		$text .= "$langHowToResetTitle";

		while ($s = $res_stm->fetch(PDO::FETCH_ASSOC)) {
