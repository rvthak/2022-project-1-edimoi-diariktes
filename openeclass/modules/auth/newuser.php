...
if (!isset($submit)) {
	...
	(user_id, nom, prenom, username, password, email, statut, department, am, registered_at, expires_at, lang)
	VALUES ('NULL', ?, ?, ?, ?, ?,?,?,?,?,?,?)";
	$last_id = db_query_safe($q1,array($nom_form,$prenom_form,$uname,$password_encrypted,$email,'5',$department,$am,$registered_at,$expires_at,$lang),$mysqlMainDb)->lastInsertId();
	...
	db_query_safe("INSERT INTO loginout (loginout.idLog, loginout.id_user, loginout.ip, loginout.when, loginout.action)
	VALUES ('', ?,?, NOW(), 'LOGIN')",array($uid,$REMOTE_ADDR),$mysqlMainDb);
	$_SESSION['uid'] = $uid;
	$_SESSION['statut'] = 5;
