...
$submit = htmlspecialchars($submit, ENT_QUOTES);
...
if($submit) {
	...
	$proflanguage = htmlspecialchars($proflanguage, ENT_QUOTES);
	...
	if (!$all_set) {
		...
		$inscr_user = db_query_safe("INSERT INTO user
				(nom, prenom, username, password, email, statut, department, am, registered_at, expires_at,lang)
				VALUES (?,?,?,?,?,?,?,?,?,?,?)",
			array($nom_form,$prenom_form,$uname,$password_encrypted,$email_form,$pstatut,$depid,$comment,$registered_at,$expires_at,$proflanguage),
			$mysqlMainDb);
		// close request
