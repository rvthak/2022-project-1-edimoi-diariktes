...
if( isset($codefaculte)){
	$codefaculte = htmlspecialchars($codefaculte, ENT_QUOTES);
}
if( isset($faculte)){
...
elseif ($a == 1)  {
	if (isset($add)) {
		...
		// OK Create the new faculty
			...
			$codefaculte = htmlspecialchars($codefaculte, ENT_QUOTES);
			$faculte = htmlspecialchars($faculte, ENT_QUOTES);
			( is_null( db_query_safe(
				"INSERT into faculte(code,name,generator,number) VALUES(?,?,'100','1000')",
				array($codefaculte, $faculte), $mysqlMainDb
			) ) == FALSE )
...
elseif ($a == 3)  {
	...
	if (isset($_POST['edit'])) {
		...
		elseif (/*mysql_num_rows(mysql_query("SELECT * from faculte WHERE id <> $c AND name=" .
                                                   autoquote($faculte)))*/
		db_query_safe("SELECT * from faculte WHERE id <> $c AND name=?",array($faculte),$mysqlMainDb,FALSE )->rowCount() 
		> 0) {
			$tool_content .= "<p>".$langFaculteExists."</p><br>";
			$tool_content .= "<p align='right'><a href='$_SERVER[PHP_SELF]?a=3&amp;c=$c'>$langReturnToEditFaculte</a></p>";
		} else {
		// OK Update the faculte
			...
			(is_null(db_query_safe("UPDATE faculte SET name = ? WHERE id=?",array($faculte,$c),$mysqlMainDb)) == FALSE)
				or die ($langNoSuccess);
		// For backwards compatibility update cours and cours_facult also
			(is_null(db_query_safe("UPDATE cours SET faculte = ? WHERE faculteid=?",array($faculte,$c),$mysqlMainDb)) == FALSE)
			...
			(is_null(db_query_safe("UPDATE cours_faculte SET faculte = ? WHERE facid=?",array($faculte,$c),$mysqlMainDb)) == FALSE)
				or die ($langNoSuccess);
		...
		$myrow = db_query_safe("SELECT code, name FROM faculte WHERE id=?",array($c),$mysqlMainDb,FALSE)->fetch();
