...
$urlText = htmlspecialchars($urlText, ENT_QUOTES);
...
if (isset($submit) && (!isset($ldap_submit)) && !isset($changePass)) {
    ...
    $username_check=db_query_safe(
        "SELECT username FROM user WHERE username=?",
        array(escapeSimple($username_form)),
        $mysqlMainDb,
        FALSE
    );
	while ($myusername = $username_check->fetch()/*mysql_fetch_array($username_check)*/)
	{
	...
	// everything is ok
        ...
        !is_null( db_query_safe(
	        ...
	        username=?, email=?, am=?,
	            perso=?, lang=?
	        WHERE user_id=?",
            array($nom_form,$prenom_form,$username_form,$email_form,$am_form,$persoStatus,$langcode,$_SESSION["uid"]),
            $mysqlMainDb
        ) ) 
        ) {
...
/*	mysql_query("UPDATE user SET perso = '$persoStatus',
	...
	db_query_safe("UPDATE user SET perso = ?,
    lang = ? WHERE user_id=? ",
    array($persoStatus,$langcode,$_SESSION["uid"]),$mysqlMainDb);
...
if(isset($msg))
{
	switch ($msg){
		...
		case 3: { //pass too easy
            $_SERVER['REMOTE_ADDR'] = htmlspecialchars($_SERVER['REMOTE_ADDR'], ENT_QUOTES);
...
$myrow = db_query_safe($sqlGetInfoUser,array($uid),$mysqlMainDb,FALSE)->fetch();
...
$password_form = htmlspecialchars($password_form, ENT_QUOTES);
$email_form = htmlspecialchars($email_form, ENT_QUOTES);
$am_form = htmlspecialchars($am_form, ENT_QUOTES);
$persoStatus = htmlspecialchars($persoStatus, ENT_QUOTES);
$userLang = htmlspecialchars($userLang, ENT_QUOTES);
