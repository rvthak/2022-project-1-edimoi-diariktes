...
if (isset($submit) && isset($changePass) && ($changePass == "do")) {
	...
	$sql = "SELECT `nom`,`prenom` ,`username`,`email`,`am` FROM `user`WHERE `user_id`= ? ";
	//$result = db_query($sql, $mysqlMainDb);
	//$myrow = mysql_fetch_array($result);
	$result_stm = db_query_safe($sql,array($_SESSION["uid"]), $mysqlMainDb, FALSE);
	$myrow = $result_stm->fetch();//mysql_fetch_array($result);
	...
	if($old_pass == $old_pass_db) {
		...
		$sql = "UPDATE `user` SET `password` = '$new_pass' WHERE `user_id` =?";
		//db_query($sql, $mysqlMainDb);
		db_query_safe($sql,array($_SESSION["uid"]), $mysqlMainDb);
