...
$forum_name = htmlspecialchars($myrow["forum_name"], ENT_QUOTES);
$forum_access = htmlspecialchars($myrow["forum_access"], ENT_QUOTES);
$forum_type = htmlspecialchars($myrow["forum_type"], ENT_QUOTES);
$forum_id = $forum;
...
if (isset($submit) && $submit) {
	...
	$sql = "INSERT INTO topics (topic_title, topic_poster, forum_id, topic_time, topic_notify, nom, prenom)
			VALUES ( ?, ?, ?, ?, ?, ?, ?)";
	$result_pdo = db_query_safe($sql,
							array($subject,$uid,$forum,$time,1,$nom,$prenom),
							$currentCourseID);



	$topic_id = $result_pdo->lastInsertId(); //$topic_id = mysql_insert_id();
	...
	$sql = "INSERT INTO posts (topic_id, forum_id, poster_id, post_time, poster_ip, nom, prenom)
			VALUES (?, ?, ?, ?, ?, ?, ?)";
	$result_pdo = db_query_safe($sql, array($topic_id,$forum,$uid,$time,$poster_ip,$nom,$prenom), $currentCourseID);
	if ( is_null($result_pdo) ) {
		$tool_content .= $langErrorEnterPost;
		draw($tool_content, 2, 'phpbb', $head_content);
		exit();
	} else {
		$post_id = $result_pdo->lastInsertId(); //$post_id = mysql_insert_id();
		if ($post_id) {
			...
			$sql = "INSERT INTO posts_text (post_id, post_text)
					VALUES (?,?)";
			$result = db_query_safe($sql, array($post_id,$message), $currentCourseID);
			...
			$sql = "UPDATE topics
				...
				WHERE topic_id = ?";
			$result = db_query_safe($sql, array($post_id, $topic_id), $currentCourseID);
		}
	}
	/*$sql = "UPDATE forums
	...
	$sql = "UPDATE forums
		...
		WHERE forum_id = ?";
	$result = db_query_safe($sql, array($post_id, $forum), $currentCourseID);
