...
if (!isset( $_POST['authors']) || !isset( $_POST['description']))
	...
	if (!$error) {
		...
		$dropbox_filename = htmlspecialchars($_FILES['file']['name'], ENT_QUOTES);
		$dropbox_filesize = htmlspecialchars($_FILES['file']['size'], ENT_QUOTES);
		$dropbox_filetype = htmlspecialchars($_FILES['file']['type'], ENT_QUOTES);
		$dropbox_filetmpname = htmlspecialchars($_FILES['file']['tmp_name'], ENT_QUOTES);
		...
		if (!$error) {
            ...
            $dropbox_filename = filename_chiper($dropbox_filename).".txt";
			...
			if (!$error)
			{
				...
				new Dropbox_SentWork($uid, 
                    htmlspecialchars($dropbox_title, ENT_QUOTES), 
                    htmlspecialchars($_POST['description'], ENT_QUOTES), 
                    htmlspecialchars($_POST['authors'], ENT_QUOTES), 
                    htmlspecialchars($dropbox_filename, ENT_QUOTES),                     
                    $dropbox_filesize, 
                    $newWorkRecipients
                );
