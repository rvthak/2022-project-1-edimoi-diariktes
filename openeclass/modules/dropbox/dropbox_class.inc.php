...
class Dropbox_Work {
	...
	function _createNewWork ($uploaderId, $title, $description, $author, $filename, $filesize) {
		...
		*/
		if ($this->isOldWork) {
			...
			$result = db_query_safe(
				$sql,
				array($this->filesize,$this->title,$this->description,$this->author,$this->lastUploadDate,$this->id,$currentCourseID)
			);
			/*$result = db_query($sql);*/
		} else {
			...
			$this->id = $result_pdo->lastInsertId(); //$this->id = mysql_insert_id(); //get automatically inserted id
		}
		...
		*/
		...
		$sql="INSERT INTO `".$dropbox_cnf["personTbl"]."` 
				(fileId, personId) VALUES (?, ?)";
        $result = db_query_safe($sql,array($this->id,$this->uploaderId),$currentCourseID);
	...
	function _createNewSentWork ($uploaderId, $title, $description, $author, $filename, $filesize, $recipientIds) {
		...
		*/
		foreach ($this->recipients as $rec) {	
			...
			$sql="INSERT INTO `".$dropbox_cnf["postTbl"]."` (fileId, recipientId) VALUES (?, ?)";
	        //$result = db_query($sql,$currentCourseID);	//if work already exists no error is generated
			$result = db_query_safe($sql,array($this->id,$rec["id"]),$currentCourseID);
