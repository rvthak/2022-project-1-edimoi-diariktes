...
function is_admin($username, $password, $mysqlMainDb) {
	...
	$r = mysql_query("SELECT * FROM user, admin WHERE admin.idUser = user.user_id
        ...
        /*$t_conn = new PDO("mysql:host=$mysqlServer;dbname=$dbname;host=localhost", $mysqlUser, $mysqlPassword);
        $t_conn->exec("set names utf8mb4");
	$safe_stmt = $t_conn->prepare("SELECT * FROM user, admin WHERE admin.idUser = user.user_id AND user.username = ? AND user.password = ?");
        $safe_stmt->bindParam(1,$username); $safe_stmt->bindParam(2,$password);*/
        
        /*$r_stm = db_query_safe(
        "SELECT * FROM user, admin WHERE admin.idUser = user.user_id AND user.username = ? AND user.password = ?",
        array($username,$password),$mysqlMainDb,FALSE);*/
